...
export function TarjetaCreditoPanel({ result }: { result: TcResult }) {
  ...
  const cards = [
    { label: "Facturas por TC (consumo)", value: cop(r.totalConsumo), sub: `${r.nAdq} cargos · ${r.nEnlazadas} enlazados a factura` },
    ...
    <div className="mt-10">
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-line bg-white shadow-sm">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              {["Fecha abono", "Tarjeta / Red", "Factura(s)", "Período", "Valor factura", "Bia créditos", "Consumo (TC)", "Comisión", "Neto", "Estado"].map((h) => (
          ...
          <tbody>
            {result.detalle.map((d, i) => (
              <tr key={i} className="hover:bg-primary-light/40">
                ...
                <td className="whitespace-nowrap border-b border-line px-3 py-2 text-xs text-ink-soft">{d.tipoTarjeta || d.red}<br />{d.tarjeta}</td>
