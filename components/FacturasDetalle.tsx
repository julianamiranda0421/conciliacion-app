...
import type { FacturaDetalleRow } from "@/lib/db";
import { fmtDate, signClass, money } from "@/lib/format";

// Detalle de TODAS las facturas del período (pagadas o no): status, fecha de pago,
// bia créditos, valores y contra qué cuenta cruzó el pago (al final).
  ...
  return (
    <div className="mt-6 rounded-xl border border-line bg-white p-5 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center gap-3">
        ...
        <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className="h-9 rounded-md border border-line bg-white px-3 text-sm">
          ...
          <div className="max-h-[600px] overflow-auto">
            <table className="w-full border-collapse">
              <thead className="sticky top-0">
                <tr>
                  {["Factura", "Período", "Status factura", "Fecha de pago", "Valor factura", "Valor aplicado", "Bia créditos", "Cuenta cruce"].map((h) => (
                ...
                {filtered.map((r, i) => {
                  ...
                  return (
                    <tr key={`${r.billId}-${i}`} className="hover:bg-primary-light/40">
                      ...
                      <td className={td}>
                        <span className={`rounded-md px-2 py-1 text-xs font-bold ${statusOk ? "bg-success/15 text-success" : "bg-warning/20 text-warning"}`}>{r.status ?? "—"}</span>
                      </td>
